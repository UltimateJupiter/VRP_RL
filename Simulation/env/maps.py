...
class DukeMap():
    ...
    def __init__(self, csv_fl, frame_rate=3, skip_station=False):
        self.frame_rate = frame_rate
        self.center_lat, self.center_lat = -1, -1
        self.nodes = []
        ...
        self.xspan = max([node.crd[0] for node in self.nodes])
        self.yspan = max([node.crd[1] for node in self.nodes])
        return
    ...
    def parse_csv(self, csv_fl):
        ...
        self.center_long = (max(crds_long) + min(crds_long)) / 2
        self.center_lat = (max(crds_lat) + min(crds_lat)) / 2
        self.correction_long = np.cos(self.center_lat / 180 * np.pi)
        circ = 1.0015e7 / 90.0

        for i, node_info in enumerate(map_data):
            name = node_info[1]
            crd = np.array([(float(node_info[3]) - self.center_long) * circ * self.correction_long, (float(node_info[2]) - self.center_lat) * circ])
            is_station = name[0] != 'n'
    ...
    def edges(self):
        if self.edges_cache != []:
            return self.edges_cache
        edges = []
        for node in self.nodes:
            for neighbor in node.neighbors:
                edge = sorted([node.ind, neighbor.ind])
