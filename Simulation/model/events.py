...
class DiffEvent():
    def __init__(self, M, self_ind, target_ind, t_peak_mean : int, dist_mean : int, flow_mean : int, t_peak_coef_std=10, dist_coef_std=0.2, flow_coef_std=0.2, span=4):
        ...
        self.t_peak_mean = t_peak_mean * 60 * 3
...
class UniRandEvent():
    ...
    def __init__(self, M, self_ind : int, weights : np.array, coef_std=0.1):
        ...
        self.weights_init = weights / 60 / 3 # normalize weight to each frame
...
class GaussRandEvent():
    ...
    def __init__(self, M, self_ind : int, weights : np.array, center : float, std : float, coef_std=0.1):
        ...
        self.self_ind = self_ind
        self.weights_init = weights / 60 / 3 # normalize weight to each frame
        self.centers_init = center * np.ones_like(self.weights_init) * 3 * 60
        self.stds_init = std * np.ones_like(self.weights_init) * 3 * 60
    ...
    def __str__(self):
        return "GaussianRandEV {} | center {:.4g} | std {:.4g} | weights {}".format(self.self_ind, self.centers[0] / 60 / 3, self.stds[0] / 60 / 3, self.weights)


def vis_event_uni():
    
    ev = UniRandEvent(None, 1, np.array([200, 0, 1000]))
    density = 60 * 3
    t_all = 24 * 60 * 3
    time = np.arange(t_all // density) / (3 * 60) * density
    res = np.zeros_like(time)
    for t in range(t_all):
        res[t // density] += ev.get_flow(t)[0]
    print(time)
    plt.bar(time, res, width=1 / (3 * 60) * density)
    plt.show()

def vis_event_gaussian():
    
    ev = GaussRandEvent(None, 1, np.array([200, 0, 1000]), 12, 4)
    density = 60 * 3
    t_all = 24 * 60 * 3
    time = np.arange(t_all // density) / (3 * 60) * density
    res = np.zeros_like(time)
